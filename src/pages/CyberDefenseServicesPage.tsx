  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#F8F9FA" }}>
      ...
      <section className="py-24" style={{ backgroundColor: "#F8F9FA", borderTop: "1px solid rgba(59,59,57,0.08)" }}>
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl md:text-4xl font-bold mt-4 mb-4" style={{ color: "#3B3B39" }}>How We Work.</h2>
            <p className="max-w-2xl mx-auto" style={{ color: "#646464" }}>A proven methodology that transforms security from a problem into a strategic advantage.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto relative">
            <div className="hidden md:block absolute top-[72px] h-px" style={{ left: '16%', right: '16%', backgroundColor: "rgba(59,59,57,0.15)" }} />
            {threeAs.map((item) => (
              <div key={item.title} className="text-center relative">
                <div className="w-36 h-36 flex items-center justify-center mx-auto mb-6 relative z-10" style={{ backgroundColor: "rgba(59,59,57,0.05)", border: "2px solid rgba(59,59,57,0.15)" }}>
                  <item.icon className="w-12 h-12" style={{ color: "#F36F21" }} strokeWidth={1} />
                </div>
                <span className="text-xs uppercase tracking-[0.15em] font-medium" style={{ color: "#646464" }}>{item.step}</span>
                <h3 className="text-2xl font-bold mt-2 mb-2" style={{ color: "#3B3B39" }}>{item.title}</h3>
                <span className="text-xs uppercase tracking-[0.15em] font-medium" style={{ color: "#F36F21" }}>{item.tagline}</span>
                <p className="leading-relaxed mt-4 max-w-xs mx-auto" style={{ color: "#646464" }}>{item.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20" style={{ backgroundColor: "#F8F9FA", borderTop: "1px solid rgba(59,59,57,0.08)" }}>
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4" style={{ color: "#3B3B39" }}>Advisory Services</h2>
            <p className="max-w-2xl mx-auto" style={{ color: "#646464" }}>Three ways we help you build executive-level security leadership.</p>
          </div>
      ...
      <section className="py-20" style={{ backgroundColor: "#F8F9FA", borderTop: "1px solid rgba(59,59,57,0.08)" }}>
        <div className="container mx-auto px-4">
          <div className="max-w-3xl mx-auto text-center">
            <h3 className="text-2xl md:text-3xl font-bold mb-4" style={{ color: "#3B3B39" }}>Ready for Executive-Level Security Leadership?</h3>
            <p className="mb-8" style={{ color: "#646464" }}>Start with a free discovery call to discuss your advisory needs.</p>
