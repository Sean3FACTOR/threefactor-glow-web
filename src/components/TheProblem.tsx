  ...
  return <section className="py-20 relative bg-muted/30 border-t border-border">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mb-16 mx-0 text-left bg-transparent font-normal">
          ...
          <h3 className="text-2xl md:text-3xl font-semibold text-foreground border-l-4 border-primary pl-6">If your compliance is only "paper-thin," you are one audit, or one breach away from the "Triple Penalty":</h3>
        </div>

        {/* Cascading Cards - Left Aligned */}
        <div className="max-w-5xl space-y-6">
          {penalties.map((penalty, index) => <div key={penalty.title} className="group flex flex-col md:flex-row bg-card border border-border rounded-xl overflow-hidden transition-all duration-300 hover:border-primary/50 hover:shadow-lg" style={{
          marginLeft: `${index * 1.5}rem`
              ...
              <div className="w-full md:w-[35%] shrink-0 relative overflow-hidden">
                <img src={penalty.image} alt={penalty.imageAlt} className="w-full h-48 md:h-full object-cover transition-all duration-500 group-hover:scale-105" />
              </div>
            </div>)}
        </div>
      </div>
