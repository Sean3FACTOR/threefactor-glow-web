...
const PainPoints = () => {
  return <section className="py-20 relative bg-background border-t border-border">
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mb-16">
          ...
          <h3 className="text-2xl md:text-3xl font-semibold text-foreground border-l-4 border-primary pl-6">In enterprise sales, compliance isn't a suggestion. It's a gatekeeper.</h3>
        </div>

        {/* 3x2 Grid Layout */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {painPoints.map(point => <div key={point.title} className="group flex flex-col bg-card border border-border rounded-xl overflow-hidden transition-all duration-300 hover:border-primary/50 hover:shadow-lg">
              {/* Image - Top */}
              <div className="w-full h-48 shrink-0 relative overflow-hidden">
                <img src={point.image} alt={point.imageAlt} className="w-full h-full object-cover transition-all duration-500 group-hover:scale-105 img-grayscale-hover" />
              </div>
